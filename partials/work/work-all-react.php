<?php /* react projects */ ?>
<!-- react 1 -->
<article id="react-1" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">IP Info</h3>
				<div class="content">
					<p>This is a React app that interacts with <a href="https://ipinfo.io/" rel="nofollow" target="_blank">IPinfo API</a> to get the user IP number and information about the IP number such as the country of origin, region and city.
                    </p>
                    <span>Screenshot been taken with VPN on</span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://ipinfo.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="//github.com/tim-green/ip-info-react" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-ipinfo.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-ipinfo.webp" width="800" height="600" alt="React IPinfo">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 2 -->
<article id="react-2" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">The Password Generator</h3>
				<div class="content">
					<p>This is a secure password and passphrase generator built with React
                    </p>
                    <p>It also features some helpful hints that help you keep good password hygeiene. This app doesn't keep any information</p>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://pw-gen.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="//github.com/tim-green/password-generator" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-pwgen-1.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-pwgen-1.webp" width="800" height="600" alt="React IPinfo">
					</div>
					<div class="gallery-item dark">
                    <img class="gallery-lazy" data-src="../assets/images/react-slides/react-pwgen-2.webp" width="800" height="600" alt="React IPinfo">
					</div>
                    <div class="gallery-item dark">
                    <img class="gallery-lazy" data-src="../assets/images/react-slides/react-pwgen-3.webp" width="800" height="600" alt="React IPinfo">
					</div>
                    <div class="gallery-item dark">
                    <img class="gallery-lazy" data-src="../assets/images/react-slides/react-pwgen-4.webp" width="800" height="600" alt="React IPinfo">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 3 -->
<article id="react-3" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Film Base</h3>
				<div class="content">
					<p>A React that fetches the data from TMDB (The Movie Database) API. It also displays the movie rating, budget, runtime and official site if it is avialable. 
                    </p>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://movies.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/movdab" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-filmbase.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-filmbase.webp" width="800" height="600" alt="">
					</div>
                    <div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-filmbase-2.webp" width="800" height="600" alt="">
					</div>
                    <div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-filmbase-3.webp" width="800" height="600" alt="">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>

<?php /*

<!-- react 4 -->
<article id="react-4" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">RT Ipsum</h3>
				<div class="content">
					<p>I’m a fan of a company called Rooster Teeth, there are an online video production company that provides myself with some great entertainment
					</p>
					<p>
Many of the onscreen ‘talent’ have many remomorable quotes, so I’ve built a Lorem Ipsum generator for the times I need some filler text.
                    </p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="https://rt-ipsum.herokuapp.com" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/rt-ipsum" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-rt.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-rt.webp" width="800" height="600" alt="Rooster Teeth Lorem Ipsum generator">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-rt-1.webp" width="800" height="600" alt="Copied filler text">
					</div>
                    
				</div>
			</div>
		</div>
	</div>
</article>
*/?>

<!-- react 5 -->
<article id="react-5" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">GroundCTRL React UI</h3>
				<div class="content">
					<p>As I'm reading, learning up and understand UI and UX principles along the side of learning React I decided to build a React based User Interface Dashboard
                    </p>
					<p>The content in this Dashboard is using the filler text from my RT Lorem Ipsum React app..</p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://groundctrl-ui.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/groundctrl-ui" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-groundctrl.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-groundctrl.webp" width="800" height="600" alt="ground control admin dashboard">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-groundctrl-1.webp" width="800" height="600" alt="Demo of graph within ground control">
					</div>
                    <div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-groundctrl-2.webp" width="800" height="600" alt="Font size level headings">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 6 -->
<article id="react-6" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Emoji Search</h3>
				<div class="content">
					<p>This is a super simple emoji search tool made with React. It features 20 different emojis when launching the app.
                    </p>
					<p>
					If you wanting to view more emojis you simply can search for Santa (for example), and instantly you'll see all the various Santa based emoticons.
					</p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://emoji.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/emoji-search-react" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-emoji.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-emoji.webp" width="800" height="600" alt="emoji react search function">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>

<!-- react 7 -->
<article id="react-7" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Snake Game</h3>
				<div class="content">
					<p>This is for anyone who had a Nokia phone way back in the late 90's or early 2000's just like myself. And loved to play that Snake game
                    </p>
					<p>So I've created a React+Redux based verison of that classic game. To play just use the arrows keys on your keyboard but remember not to hit the wall, you just have three hearts.</p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://snake.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/react-snake-game" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-snake.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-snake.webp" width="800" height="600" alt="Classic snake game">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-snake-2.webp" width="800" height="600" alt="demo of the snake game">
					</div>
                    <div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-snake-1.webp" width="800" height="600" alt="game on pause">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 8 -->
<article id="react-8" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Tetris Rush</h3>
				<div class="content">
					<p>Another game I like to play is Tetris. So here is another React app that I've built for those wants to match tiles.
                    </p>
					<p>To play this game you only have 5 minutes to play the game and get the highest score you. Like the Snake game above you use the arrow keys on your keyboard.</p>
					
					<p>Your score is stored in localStorage.</p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://tetris.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/tetris-rush" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-tetris.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-tetris.webp" width="800" height="600" alt="Tetris rush you've got five minutes">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-tetris-1.webp" width="800" height="600" alt="game in progress">
					</div>
                    <div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-tetris-2.webp" width="800" height="600" alt="new record in tetris rush">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-tetris-3.webp" width="800" height="600" alt="game over">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 9 -->
<article id="react-9" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Splash</h3>
				<div class="content">
					<p>This is a clone of the stock photography site called <a href="https://unsplash.com/" rel="nofollow">Unsplash</a>.</p> 
					
					<p>It features a simple layout to display all the images. The app uses the <a href="https://unsplash.com/developers" rel="nofollow">Unsplash API</a> to grab all the images. 
                    </p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://unsplash.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/splash-clone" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-unsplash.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-unsplash.webp" width="800" height="600" alt="finding photo">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-unsplash-1.webp" width="800" height="600" alt="selected photo within a lightbox with information">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 10 -->
<article id="react-10" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">Todoist Clone</h3>
				<div class="content">
					<p>A Clone of the popular Task Manager App Todoist. </p>
					
					<p>Using Firestone to store all the to do's and while been built with React it also uses React hooks, useState, useEffect. 
                    </p>
                    <span></span>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://todo-demo.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/todoist-clone-react" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-todo.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-todo.webp" width="800" height="600" alt="to do list">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-todo-1.webp" width="800" height="600" alt="mobile view of the todo list">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>


<!-- react 11 -->
<article id="react-11" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">The Colour Contrast Checker</h3>
				<div class="content">
					<p>This is a React app that checks the colour contrast of your design based on the Web Content Accessibility Guidelines (WCAG). </p>
					
					<p>Features on this app are that it's accept HEX, RGB and HSL colour formats and converts them. If you like any colour contrasts you can save them as it can store those colours combinations in your local storage. 
                    </p>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://colour.timgreen.xyz/" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="https://github.com/tim-green/colour-contrast-checker" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-colour-contrast.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-colour-contrast.webp" width="800" height="600" alt="to do list">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-colour-contrast2.webp" width="800" height="600" alt="mobile view of the todo list">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>

<?php /* 
<!-- react 12 -->
<article id="react-12" class="project">
	<div class="wrapper">
		<header class="project-overview expandable">
			<div>
				<p class="overline">React</p>
				<h3 class="large-heading">COVID-19 Dashboard</h3>
				<div class="content">
					<p>This is a React dashboard showing the current COVID-19 world and country stats. The COVID-19 Data is sourced from  <a href="https://github.com/CSSEGISandData/COVID-19" rel="nofollow" target="_blank">Johns Hopkins CSSE</a>.
                    </p>
				</div>
				<dl class="details">
					<dt>Visit</dt>
					<dd><a href="http://covid19.timgreen.xyz" target="_blank">View Project</a></dd>
					<dt>Git Repo</dt>
					<dd><a href="//github.com/tim-green/react-covid19-dash" target="_blank">View Repo</a></dd>
				</dl>
			</div>
		</header>
		<div class="project-content">
			<div class="gallery">
				<div class="gallery-placeholder">
					<img class="lazy-inline" data-src="../assets/images/react-slides/react-covid.webp" width="800" height="600" alt="Loading Image...">
				</div>
				<div class="gallery-wrapper">
					<div class="gallery-item main dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-covid.webp" width="800" height="600" alt="covid 19 react app">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-covid2.webp" width="800" height="600" alt="covid 19 react app stats">
					</div>
					<div class="gallery-item dark">
						<img class="gallery-lazy" data-src="../assets/images/react-slides/react-covid3.webp" width="800" height="600" alt="covid 19 react app data">
					</div>
				</div>
			</div>
		</div>
	</div>
</article>
*/?>