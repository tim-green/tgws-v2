...
const config = {
	root: null, // the setting root to null sets it to viewport
	rootMargin: '0px',
	threshold: 0.5
}
...
animateObserver = new IntersectionObserver((entries) => {
	entries.forEach(entry => {
		...
		// } else { entry.target.classList.remove('play');
		}
	})
}, config);
...
//prevents downloads for future project with media value
var media = document.querySelectorAll(".prevent-download");
media.forEach(t => {
    t.src ? (t.setAttribute("data-src", t.src), t.removeAttribute("src")) : t.srcset && (t.setAttribute("data-srcset", t.srcset), t.removeAttribute("srcset"))
});

//assorted JS functions for site
! function(e, o) {
